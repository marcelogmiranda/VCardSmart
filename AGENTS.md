...
- **SPM DESATIVADO no projeto (11/08)**: `pubspec.yaml` → `flutter: config: enable-swift-package-manager: false`. Motivo: Flutter 3.44 ativa Swift Package Manager por padrão e migra `webview_flutter_wkwebview` para SPM; mas `google_mobile_ads` 9.1.0 (CocoaPods+SPM) depende dele via podspec; `flutter_secure_storage`, `mobile_scanner`, `nfc_manager` ainda são CocoaPods-only → build iOS do CI falhava com SPM. Com o opt-out, o CI usa CocoaPods puro (igual ao local). Ponte até Dec/2026 (CocoaPods read-only); depois será preciso reativar SPM quando todos os plugins suportarem. Não rodar `flutter config --no-enable-swift-package-manager` na CI — o opt-out no pubspec é suficiente e commitável.
...
- Status 1.0.9/17 (10/09): **IPA** `~/Downloads/VCardSmart-1.0.9-17.ipa` (34MB, SHA-256 `2216b373...6304`). Fix crash iPadOS 27 (upgrade AdMob 4→9) + remoção NDEF entitlement. Respondendo reviewer App Store (Guideline 2.1 + 2.1(a)).
...
### Estado v1.0.9 (10/09) — upgrade AdMob + fix crash iPadOS 27 + NDEF
- pubspec **1.0.9+17** (numeração única Play/iOS); `AppConstants.appVersion` = '1.0.9+17' (e teste).
- **google_mobile_ads 4.0.0 → 9.1.0**: upgrade de 5 major versions para resolver **crash no iPadOS 27** (App Store review Guideline 2.1(a)). O SDK nativo `Google-Mobile-Ads-SDK` foi de **10.11.0** (sem suporte iOS 26+) para **13.9.0** (com suporte iOS 27). A versão 4.0.0 está deprecated desde fevereiro/2025 (sunset Q2/2026). A 9.1.0 adiciona: SPM support, UISceneDelegate migration, Ad Preloading APIs. **APIs usadas não mudaram** (`MobileAds.initialize()`, `BannerAd`, `AdSize.banner`, `AdRequest`, `AdWidget`).
- **NDEF removido do `Runner.entitlements`**: mantidos apenas `PACE` + `TAG`. A App Store rejeita NDEF (`Asset validation failed 90778`). Entitlements verificados via `codesign -d --entitlements`: somente PACE+TAG.
- **Workaround `CLANG_ALLOW_NON_MODULAR_INCLUDES_IN_FRAMEWORK_MODULES = YES`**: bug conhecido do `google_mobile_ads 9.1.0` com CocoaPods — o plugin inclui `GoogleMobileAds_Beta.h` de `PrivateHeaders/` do framework. Setting adicionado ao Runner target (Debug/Release/Profile no pbxproj) E ao post_install do Podfile (p/ Pods project).
- **NFC entitlement iOS**: `Runner.entitlements` agora contém apenas `PACE` + `TAG` (removido `NDEF`). Profile de distribuição no portal pode conter NDEF+TAG+PACE, mas o entitlement embarcado no `.app` é o que define o comportamento.
- **Testes**: `flutter analyze` → No issues found; suíte determinística **480 testes verdes**.
- **Builds**: **IPA 1.0.9/17** em `~/Downloads/VCardSmart-1.0.9-17.ipa` (34MB, SHA-256 `2216b373...6304`; CFBundle 1.0.9/17, Min iOS 15.0, entitlement NFC = `TAG` — validado). Archive: `build/ios/archive/Runner.xcarchive` (export via workaround xcodebuild).
- **SPM note**: `enable-swift-package-manager: false` continua necessário — `google_mobile_ads 9.1.0` suporta SPM mas `flutter_secure_storage`, `mobile_scanner`, `nfc_manager` ainda são CocoaPods-only.

### Estado v1.0.10 (10/09) — fallback de gravação NFC (tamanho vs capacidade)
- pubspec **1.0.10+18** (numeração única Play/iOS); `AppConstants.appVersion` = '1.0.10+18' (e teste). Commit `356f080` (v1.0.8+16) era o último — rodadas 1.0.9 e 1.0.10 commitadas juntas.
- **Causa raiz do erro do usuário no iPhone**: ao gravar um cartão NTAG213 (~144 bytes NDEF), o **vCard completo** (com bio/socials) estourava a capacidade → mensagem "tamanho do perfil e capacidade de gravação". **NÃO era falha de entitlement** (isso já tinha sido resolvido e a gravação funcionava).
- **Fix — fallback progressivo** em `sendData` (`nfc_datasource.dart`): candidatos em ordem de preferência → **vCard completo → vCard mínimo (FN/TEL/EMAIL/URL, `encodeMinimalVCard`) → registro URI com o link do perfil (`profileUrl`: website > instagram > linkedin > facebook > x > social; trata `@handle`→`https://instagram.com/...`)**. No `onDiscovered`, escolhe o primeiro com `byteLength ≤ maxSize`; só erra se nem o URI couber. Novo param `sendData(data, {Profile? profile})`; repositório passa o perfil.
- **Sobrescrita confirmada (09/09)**: `Ndef.write()` substitui o conteúdo NDEF inteiro (o cartão do Instagram virou o vCard do perfil, "Perfil gravado com sucesso!" no iPhone). Cartão protegido/read-only → erro amigável já existente.
- **Instrumentação**: `debugPrint('[NFC-send]/[NFC-read] ...')` nos caminhos de erro/onError e no write escolhido (diagnóstico futuro). NOTA: no **profile mode o devicectl console NÃO mostra o debugPrint** (confirmado) — para ver logs é preciso `flutter run` (debug, com VM service) ou release install.
- **Testes**: +7 (3 datasource fallback + 4 converter minimal/URL) → suíte determinística **487 testes verdes**; `flutter analyze` → No issues found. Corrigido teste de versão desatualizado (esperava 1.0.8+16).
- **Builds**: **IPA 1.0.10/18** em `~/Downloads/VCardSmart-1.0.10-18.ipa` (36MB, SHA-256 `786eee19...7876b`; CFBundle 1.0.10/18, Min iOS 15.0, entitlement NFC = PACE+TAG, beta-reports-active=true). **`flutter build ipa --release` funcionou DIRETO** (sem workaround xcodebuild — provável porque o profile `VCardSmart` com NFC está instalado em `~/Library/MobileDevice/Provisioning Profiles/`). Pegar usuário: upload ao TestFlight via Transporter (pendente).

## Diagnóstico (09/09) — "NFC indisponível" no iPhone por cabo = perfil DE DESENVOLVIMENTO sem NFC
- **Causa raiz** (confirmada via `flutter build ios --debug`): o Xcode rejeita o build de dev com **`"Runner" requires a provisioning profile with the NFC Tag Reading feature`** — exatamente o "erro de requerimento" visto. O **perfil de Distribuição** `VCardSmart` (UUID `1be67110`) tem NFC, mas o **perfil de Development** (automatic signing, usado para instalar App por cabo) **NÃO** — só o de distribuição foi regenerado (AGENTS 09/09). O app instalado via cabo sai sem entitlement NFC → `readingAvailable()` false → "NFC indisponível".
- `-allowProvisioningUpdates -allowProvisioningDeviceRegistration` no xcodebuild **não resolveu** (build falhou no mesmo erro) — automatic signing via CLI não regera o perfil dev com a capability NFC.
- **Fix (2 caminhos)**: (a) abrir `ios/Runner.xcworkspace` no Xcode → Target Runner → Signing & Capabilities (Debug = automatic) → Xcode regenera o profile dev **com** NFC (1 clique) → `flutter run -d` passa; ou (b) criar **Development Profile** no portal (App ID `com.vcardsmart.app` com NFC, cert dev, device UUID `D9CA6E3D-...`) e instalar em `~/Library/MobileDevice/Provisioning Profiles/`.
- **Sessão 09/09 (parada — BLOQUEADA NA CONTA APPLE ID)**: perfil dev antigo `fe2a00db-25d4-4022-9217-5b0a7abbabf6` (`iOS Team Provisioning Profile`) **removido** de `~/Library/Developer/Xcode/UserData/Provisioning Profiles/` (não tinha NFC). Também estão lá (mas SEM NFC): `5ad7d82d` (`iOS Team Store Provisioning`) e `c21b48c2` (`VCardSmart App Store`) — os dois de App Store, obsoletos. O Xcode GUI abriu e o pbxproj foi salvo (16:25) com `CODE_SIGN_IDENTITY="Apple Development"`, `CODE_SIGN_STYLE=Automatic` e `DEVELOPMENT_TEAM=""` no **Debug target** — eu corrigi para `7775YGF9CJ` no working tree (não commitado). Entretanto `flutter build ios --debug -d` falha com **`No Accounts` / `No profiles for 'com.vcardsmart.app' were found`**: o automatic signing precisa de **Apple ID logado no Xcode** (Settings → Accounts), que **não existe** (sem Accounts, sem sessão spaceship/fastlane, sem credencial devportal no keychain). `fastlane` está instalado (`/opt/homebrew/bin/fastlane`) mas só há config p/ Android/Google Play (`fastlane/Appfile`+`Fastfile`, sem credenciais Apple). Tentativa de instalar o `.app` do archive Release (entitlement TAG) por cima via `devicectl` → **`0xe800801f` (MIInstallerErrorDomain 13 — embedded profile de App Store não instala via cabo)**. Cert dev presente: `Apple Development: Marcelo Miranda (45ZZF26V8J)`.
- **PRÓXIMO PASSO (usuário)**: logar Apple ID no Xcode (Settings → Accounts → + → Apple ID + 2FA) e avisar. Com a conta logada, `flutter build ios --debug -d <D9CA6E3D...>` deve gerar o profile dev com NFC (App ID já tem a capability) e instalar. **Alternativa**: rodar `fastlane sigh development` (pedirá Apple ID+senha+2FA no terminal) ou criar profile "iOS Development" manualmente no portal.
- **Alternativa que JÁ está pronta p/ validar NFC de verdade no iPhone**: **TestFlight** — o IPA 1.0.8/16 (entitlement `TAG`) instala no iPhone e o NFC funciona (não depende de setup Xcode).
- Store: o IPA 1.0.8/16 **já está correto** (entitlement TAG) — nenhum blocker neste diagnóstico.

## Pendências (próxima interação)
1. **Subir 1.0.10+18 às lojas e responder reviewer**: Transporter (`~/Downloads/VCardSmart-1.0.10-18.ipa`, upload pelo usuário — feito no GUI) e Play Console (AAB 1.0.10+18 a gerar). **App Store review**: responder ao reviewer indicando que o crash foi causado por `Google-Mobile-Ads-SDK 10.11.0` incompatível com iPadOS 27, corrigido com upgrade para 13.9.0; **fornecer link do vídeo NFC** (gravar na próxima interação). Programáticas: **Play**: bots de 16KB/edge/versão; **App Store**: Export Compliance "standard encryption / exempt" (template em `docs/11_Legal/22_ExportCompliance.md`), NÃO marcar "no encryption".
2. **Gravar vídeo NFC no iPhone (1.0.10/18)**: validado via cabo (gravar com fallback de tamanho, sobrescrever o cartão do Instagram, ler de volta); registrar vídeo de demonstração para o reviewer.
3. **Validar NFC real em OUTRO Android físico com NFC** (Redmi Note 8 NÃO serve — sem chip NFC): conectar via USB, instalar `app-debug.apk`, tocar "Gravar em Cartão NFC" → "Gravar no cartão" com o cartão do usuário (link do Instagram) na mão. O app reporta `isWritable` (se regravável) e grava o vCard; validar leitura em app de Contatos de outro celular. **Se o cartão estiver write-locked, obter um NTAG213/215/216 em branco.**
4. **Validar NFC no iPhone (1.0.10/18)**: TestFlight (aguardando processamento do IPA). Testar "Gravar Cartão"/"Receber Contato" com o cartão do usuário (agora com erro/posicionamento claros e leitura de registro URI do Instagram). Validar migração (.vcs via share sheet/AirDrop).
5. **Validar migração .vcs (export→import)** e foto/avatar (ResizeImage 224) e fluxo de ads — preferencialmente no aparelho com NFC / outro device.
6. **Export Compliance (App Store)**: manter "standard encryption / exempt". Template de carta em `docs/11_Legal/22_ExportCompliance.md`. NÃO marcar "no encryption".
