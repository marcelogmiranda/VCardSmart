...
1. **Subir o AAB 1.0.7/15 Android** (`flutter build appbundle --release`) com a MESMA numeração 1.0.7+15, para manter sincronia Play/iOS.
...
3. **Validar NFC no iPhone (1.0.7/15)**: build instalado via USB ou TestFlight — agora `NFCTagReaderSession.readingAvailable` deve retornar `true`. Testar "Gravar Cartão"/"Receber Contato" com o cartão do usuário. Validar migração (.vcs via share sheet/AirDrop).
